<script type="text/javascript">
    function ajaxFileUpload()
    {
        ...
        (
        {
            url:'<?=site_url('sos/pegawai/upload_foto/')?>',
            secureuri:false,
    ...
    $(document).ready(function(){
        ...
        //    ID = ID.split('_');
        //    $.post("<?=site_url('sos/pegawai/last_message/')?>/"+ID[1],
        //    function(data){
        //    if (data != "") {
        ...
        $("form#update_status").live('submit',function(event){
            ...
            var id = $(this).find('input[name=id_pegawai]').val();
            var stat = $(this).find('textarea').val();
            var gambar = $('#gambar').val();
			var profile_poto=$('#profilrpoto').val();
            $(this).find('textarea').val('');
            $("#gambar").val('');
            $("#file_upload").html('');
            if(stat!='') {
                $.post("<?=site_url('sos/pegawai/set_status')?>", { id_pegawai : id, status : stat, images : gambar, profilepoto : profile_poto }).done(function(data) {
        ...
        $("form#komentar").live('submit',function(e){ 
            e.preventDefault();
            var id_pegawai = $(this).find('input[name=id_pegawai]').val();
            ...
            if(komentar!='') {
                $.post("<?=site_url()?>sos/pegawai/set_komentar", {id_pegawai:id_pegawai,id_status:id_status,komentar:komentar,profilepoto:profile_poto}).done(function(data) {
        ...
        $(".delete_status").live('click',function(){
            ...
            if(result[0]=='status') {
               $("#hapusstatus_"+result[1]).fadeOut( 200, function() { 
                ...
                $.post("<?=site_url('sos/pegawai/del_status')?>", { id_status : result[1] }).done(function(data) {
                    console.log('sukses');    
                });
            });
            ...
            $("#hapuskomentar_"+result[1]).fadeOut( 200, function() { 
                ...
                $.post("<?=site_url('sos/pegawai/del_komentar')?>", { id_komen : result[1] }).done(function(data) {
                    console.log('sukses');    
                });
            });
        }
    ...
    <script type="text/javascript">
            $(function() {
                $('#tags_3').tagsInput({
                    ...
                    autocomplete_url:'<?=site_url('sos/pegawai/cari_teman')?>' // jquery ui autocomplete requires a json endpoint
                });
                
                $(".add").live('click',function(){
                    var id = $(this).attr('id');
                    if(id!=''){
                        $.post('<?=site_url('sos/pegawai/tambah_teman/')?>', {id_user : id }, function(data){
                             if(data=='sukses'){
                ...
                $(".lihat_pesan").live('click',function(){
                    ...
                    if(id_pesan[0]=='belum') {
                        ...
                        $.post('<?=site_url('sos/pegawai/baca_pesan')?>',{id_pesan : id_pesan[1]},function(data){
