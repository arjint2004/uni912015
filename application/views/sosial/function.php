<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        { 
            ...
            ID = ID.split('_');
            $.post("<?=site_url('sos/siswa/last_message/')?>/"+ID[1],
            function(data){
            if (data != "") {
        ...
        $("form#update_status").live('submit',function(event){
            ...
            var id = $(this).find('input[name=id_siswa]').val();
            ...
            $("#file_upload").html('');
            if(stat!='') {
                $.post("<?=site_url('sos/siswa/set_status')?>", { id_siswa : id, status : stat, images : gambar }).done(function(data) {
        ...
        $("form#komentar").live('submit',function(e){
            e.preventDefault();
            var id_siswa = $(this).find('input[name=id_siswa]').val();
            var id_status = $(this).find('input[name=id_status]').val();
            var komentar = $(this).find('textarea').val();
            $(this).find('textarea').val('');
            if(komentar!='') {
                $.post("<?=site_url()?>sos/siswa/set_komentar", {id_siswa:id_siswa,id_status:id_status,komentar:komentar}).done(function(data) {
        ...
        $(".delete_status").live('click',function(){
            ...
            if(result[0]=='status') {
               $("#hapusstatus_"+result[1]).fadeOut( 200, function() { 
                ...
                $.post("<?=site_url('sos/siswa/del_status')?>", { id_status : result[1] }).done(function(data) {
                    console.log('sukses');    
                });
            });
    ...
    <script type="text/javascript">
            $(function() {
                ...
                $("#add_all_friend").live('click',function(){
                ...
                $(".lihat_pesan").live('click',function(){
                    ...
                    if(id_pesan[0]=='belum') {
                        ...
                        $.post('<?=site_url('sos/siswa/baca_pesan')?>',{id_pesan : id_pesan[1]},function(data){
