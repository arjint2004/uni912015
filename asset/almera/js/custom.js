...
$(function () {
	...
	$('.gr-figure').on('mouseenter', function () {
		$(this).children('div.emblem').fadeOut();
	}).on('mouseleave', function () {
		$(this).children('div.emblem').fadeIn();
	});
	
	$(".errorlogin").fadeIn(1000,function(){});
	
	$(".errorlogin").delay(10000).fadeOut("slow",function(){
		$(this).remove();				
	});
	
	$('form#loginhome').submit(function(){
		$('.errorlogin').remove();
		$("header").before("<div class=\"errorlogin\"></div>");
		//$(".errorlogin").css("color","green");
		
				$(".errorlogin").delay(1000).html('Sedang proses Login');
				$(".errorlogin").css("padding-top",($(window).height()-100)/2+'px');
				$(".errorlogin").css("padding-bottom",($(window).height()-100)/2+'px');
				$(".errorlogin").append("<img id='wait'  src='"+base_url+"asset/default/images/loading.gif' />");
				$(".errorlogin").delay(1000).fadeOut("slow",function(){});
		/*$.ajax({
			...
			error: function() {
				...
				$(".errorlogin").delay(5000).html('Terjadi error atau koneksi terputus');
				$(".errorlogin").delay(5000).fadeOut("slow",function(){});
			},
			success: function(msg) {
				if(msg==1){
					...
					$(".errorlogin").css("color","red");
					$(".errorlogin").delay(5000).html('Password dan Username tidak terdaftar, mohon ulangi lagi');
					$(".errorlogin").delay(5000).fadeOut("slow",function(){
						$(this).remove();
					});					
				}
