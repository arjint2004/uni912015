...
function getCalendarByRange($id){
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "select * from `ak_jadwal` where `id` = " . $id;
  ...
  <head>    
    ...
    <link href="css/colorselect.css" rel="stylesheet" />   
      <script type="text/javascript">
		var base_url='http://<?=str_replace('jadwal/edit.php','',$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF']);?>admin';
		var id_kelas=<?=$event->id_kelas?>;
		var id_sekolah=<?=$event->id_sekolah?>;
	</script>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#Deletebtn").click(function() {
                 if (confirm("Apakah anda yakin untuk menghapus jadwal ini")) {  
           ...
           $("#stpartdate,#etpartdate").datepicker({ picker: "<button class='calpick'></button>"});    
			...
			var explstrtdate=$('#stpartdate').val().split('/');
			var explenddate=$('#etpartdate').val().split('/');
			var curStartDate=explstrtdate[2]+"-"+explstrtdate[0]+"-"+explstrtdate[1]+" "+$('#stparttime').val();
			var curEndDate=explenddate[2]+"-"+explenddate[0]+"-"+explenddate[1]+" "+$('#etparttime').val();
			
			var dateasb=converttgl();
			var waktu= dateasb+" "+stparttime+" -  "+etparttime; 
			//alert(curStartDate);
			//alert(curEndDate);
			//$('#id_pelajaranedit').load(base_url+"/jadwal/getPelajaranByIdKelas/"+id_kelas+"/"+curStartDate+"/"+curEndDate);
			$.ajax({
					type: "POST",
					data: "id_kelas="+id_kelas+"&start="+curStartDate+"&end="+curEndDate,
					url: base_url+"/jadwal/getPelajaranByIdKelas",
					beforeSend: function() {
								
					},
					success: function(msg) {
						$("#id_pelajaranedit").html(msg);				
					}
			});
			
			$.ajax({
					type: "POST",
					data: "selected=1&id_kelas="+id_kelas+"&start="+curStartDate+"&end="+curEndDate+"&waktu="+waktu,
					url: base_url+"/jadwal/getPengajar",
					beforeSend: function() {
								
					},
					success: function(msg) {
						$("#id_pegawaiedit").html(msg);				
					}
			});
        });
		function converttgl(){
				var startpartdate=$('#stpartdate').val();
				var stparttime=$('#stparttime').val();
				var etparttime=$('#etparttime').val();
				switch(startpartdate)
					{
					case '02/11/2013':
					  var dateasb="(Senin)";
					break;
		...
		function getpengajar(obj) {
				var startpartdate=$('#stpartdate').val();
				var stparttime=$('#stparttime').val();
				var etparttime=$('#etparttime').val();
			if(isLoaded != true) {
				//$('#pengajar').load(base_url+'/pengajaran/getPengajar/'+$(obj).val());
				// (Selasa) 00:30 - 01:30
					var dateasb=converttgl();
					var waktu= dateasb+" "+stparttime+" -  "+etparttime; 
				...
				$.ajax({
					type: "POST",
					data: "id_pelajaran="+$(obj).val()+"&waktu="+waktu+"&id_kelas=<?=$event->id_kelas?>",
					url: base_url+'/jadwal/getPengajar',
					beforeSend: function() {
	...
	select#id_pelajaranedit option.selected{
    ...
    <div>      
      <div class="toolBotton">           
        <a id="Savebtn" class="imgbtn" href="javascript:void(0);">                
          <span class="Save"  title="Save the calendar">Simpan(<u>S</u>)
          </span>          
        </a>                           
        <?php if(isset($event)){ ?>
      ...
      <div class="infocontainer">            
        <form action="php/datafeed.php?method=adddetails<?php echo isset($event)?"&id=".$event->Id:""; ?>" class="fform" id="fmEdit" method="post">                 
          ...
          <label>                    
            ...
            <select onchange="getpengajar(this);" name="id_pelajaran" id="id_pelajaranedit">
          ...
          <label>                    
            ...
            <select name="id_mengajarpegawai" id="id_pegawaiedit">
          ...
          <label>                    
            ...
            <div>  
              ...
              <input MaxLength="10" class="required date" id="stpartdate" name="stpartdate" style="padding-left:2px;width:90px;" type="hidden" value="<?php echo isset($event)?$sarr[0]:""; ?>" />                       
              ...
              <input MaxLength="10" class="required date" id="etpartdate" name="etpartdate" style="padding-left:2px;width:90px;" type="hidden" value="<?php echo isset($event)?$earr[0]:""; ?>" />                       
              ...
              <label class="checkp"> 
                <input id="IsAllDayEvent" name="IsAllDayEvent" type="checkbox" value="1" <?php if(isset($event)&&$event->IsAllDayEvent!=0) {echo "checked";} ?>/>          Semua Jadwal                   
              </label>                    
            </div>                
          </label>                 
          <label>                    
...
<?php echo isset($event)?$event->Description:""; ?>
          ...
          <input id="id_kelas" name="id_kelas" type="hidden" value="<?=$event->id_kelas?>" />           
          <input id="id_sekolah" name="id_sekolah" type="hidden" value="<?=$event->id_sekolah?>" />           
