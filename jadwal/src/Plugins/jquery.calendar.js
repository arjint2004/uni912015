/**
  ...
  */
  			function validate(obj,type){
				...
				if(type=='smn'){
					if($(obj).val()>59){alert('maximal menit 59');
					$(obj).val('');}
				}else{
				...
				if(type=='ejm'){
					if($(obj).val()>24){alert('maximal jam 24');
					$(obj).val('');}
				}else{
    ...
    $.fn.bcalendar = function(option) {
        ...
        function BuildWT(ht, dayarrs, events, dMax) {
            ...
            for (var i = 0; i < dayarrs.length; i++) {
				...
				var asbinsp=dayarrs[i].display.split(' ');
                ht.push("<th abbr='", dateFormat.call(dayarrs[i].date, i18n.xgcalendar.dateformat.fulldayvalue), "' class='gcweekname' scope=\"col\"><div title='", title, "' ", ev, " class='wk-dayname'><span class='", cl, "'>",asbinsp[0] , "</span></div></th>");
        ...
        function dayshow(e, data) {
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
					...
					var deldate=ss.join("");
					var deldate2=deldate.split(" ");
					//alert(deldate2[2]);
                    var ts = $("#bbit-cs-buddle-timeshow").html(deldate2[2]);
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            if (buddle.length == 0) {
                ...
                temparr.push(i18n.xgcalendar.time, ':</th><td class=cb-value><div id="bbit-cal-buddle-timeshowhide" style="display:none;"></div><div id="bbit-cal-buddle-timeshow"></div></td></tr><tr><th class="cb-key">');
                temparr.push(i18n.xgcalendar.content, ':</th><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><select id="bbit-cal-what" onclick="$(this).focus();" onchange="getpengajar(this);return false;"><option value="">Pilih Pelajaran</option></select></div></div><div class="cb-example"></div></td></tr><tr><th class="cb-key"> Guru:</th><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><input style="display:none;" id="bbit-cal-what" class="textbox-fill-input"/><select id="pengajar" onclick="$(this).focus();"><option value="0">Pilih Guru</option></select></div></div><div class="cb-example"></div></td></tr></tbody></table><input id="bbit-cal-start" type="hidden"/><input id="bbit-cal-end" type="hidden"/><input id="bbit-cal-allday" type="hidden"/><input id="bbit-cal-quickAddBTN" value="');
               // temparr.push(i18n.xgcalendar.example, '');
				...
				//<input style="display:none;" id="bbit-cal-what" class="textbox-fill-input"/>
                ...
                calbutton.click(function(e) {
					if($('#startjm').val()==''){
						$('#startjm').css('border','1px solid red');
						return false;
					}else{$('#startjm').css('border','');}
					
					if($('#startnm').val()==''){
						$('#startnm').css('border','1px solid red');
						return false;
					}else{$('#startnm').css('border','');}
					
					if($('#endjm').val()==''){
						$('#endjm').css('border','1px solid red');
						return false;
					}else{$('#endjm').css('border','');}
					
					if($('#endnm').val()==''){
						$('#endnm').css('border','1px solid red');
						return false;
					}else{$('#endnm').css('border','');}
					
					
					var valstart=$("#bbit-cal-start").val();
					var tanggal= valstart.split(' ');
					$("#bbit-cal-start").val( tanggal[0]+" "+$('#startjm').val()+":"+$('#startnm').val()+"");
					$("#bbit-cal-end").val(tanggal[0]+ " "+$('#endjm').val()+":"+$('#endnm').val()+"");
					
					
					if($('#bbit-cal-what').val()==''){
						$('#bbit-cal-what').css('border','1px solid red');
						$('#bbit-cal-what').append('<span>pelajaran harus dipilih</span>');
						//alert('Pelajaran harus di pilih');
						return false;
					}else{$('#bbit-cal-what').css('border','');}
					if($('#pengajar').val()==''){
						$('#pengajar').css('border','1px solid red');
						$('#pengajar').append('<span>pelajaran harus dipilih</span>');
						//alert('Pelajaran harus di pilih');
						return false;
					}else{$('#pengajar').css('border','');}
                    if (option.isloading) {
                        return false;
                    }
                    option.isloading = true;
                    var what = $("#bbit-cal-what").val();
                    var whatguru = $("#pengajar").val();
                    ...
                    var fguru = /^[^\$\<\>]+$/.test(whatguru);
                    if (!fguru) {
                        alert(i18n.xgcalendar.invalid_titleguru);
                        $("#pengajar").focus();
                        option.isloading = false;
                        return false;
                    }
                    ...
                    var param = [{ "name": "CalendarTitle", value: what },
						{ "name": "id_sekolah", value: id_sekolah },
						{ "name": "id_mengajar", value: whatguru },
						{ "name": "id_kelas", value: id_kelas },
			...
			var explasbin=dateshow.split('(');
			var explasbin2=explasbin[1].split(')');
			var dateexp=explasbin2[1].split('-');
			var strtasb=dateexp[0].split(':');
			var endasb=dateexp[1].split(':');
			strtasb[0]=strtasb[0].replace(' ','');
			strtasb[1]=strtasb[1].replace(' ','');
			endasb[0]=endasb[0].replace(' ','');
			endasb[0]=endasb[0].replace(' ','');
			endasb[1]=endasb[1].replace(' ','');

			
            $("#bbit-cal-buddle-timeshowhide").html("("+explasbin[1]);
            $("#bbit-cal-buddle-timeshow").html("("+explasbin2[0]+") <input onclick='$(this).focus()' onkeyup='return validate(this,\"sjm\")'  maxlength='2' type='text' id='startjm' name='startjm' size='1' value='"+strtasb[0]+"'/> : <input onclick='$(this).focus()' onkeyup='return validate(this,\"smn\")'  maxlength='2' type='text'  id='startnm' name='startnm' size='1' value='"+strtasb[1]+"'/> -  <input onclick='$(this).focus()'  onkeyup='return validate(this,\"ejm\")' maxlength='2' type='text' id='endjm' name='endjm' size='1' value='"+endasb[0]+"'/> : <input onclick='$(this).focus()' onkeyup='return validate(this,\"emn\")' maxlength='2' type='text' id='endnm' name='endnm'  size='1' value='"+endasb[1]+"'/>");
			...
			//$('#pengajar').load(base_url+"/jadwal/getPengajar/1/11/2");	
			$('#bbit-cal-what').load(base_url+"/jadwal/getPelajaranByIdKelas/"+id_kelas+"/"+semester+"");
			
			/*$("#bbit-cal-what").unbind("change",function(){
			  $('#pengajar').load(base_url+'/pengajaran/getPengajar/'+$(this).val());
			});*/
